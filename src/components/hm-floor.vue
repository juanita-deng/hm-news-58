<template>
  <div class="wrapper">
    <!-- 递归组件 -->
    <hm-floor v-if="comment.parent" :comment="comment.parent" :count="count - 1"></hm-floor>
    ...
    <div class="hm-floor" :class="{ bd: !comment.parent }">
      <div class="title">
        ...
        <div class="right" @click="replys(comment.id, comment.user.nickname)">回复</div>
      </div>
...
export default {
  ...
  methods: {
    ...
    replys(id, nickname) {
      ...
      this.$bus.$emit("reply", id, nickname); //方式二:公交车 非父子通讯
    }
  }
};
</script>
