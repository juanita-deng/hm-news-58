  ...
  <div class="hm-home">
    ...
    <van-tabs v-model="active" animated swipeable sticky>
      ...
      <div class="tab-button">
        <van-icon name="coupon-o" color="#07c160" size="40" @click="$router.push('/tab-manage')" />
      </div>
      ...
      <van-tab :title="item.name" v-for="item in tabList" :key="item.id">
        ...
        <van-pull-refresh v-model="refreshing" @refresh="onRefresh">
          ...
          >
            <!-- tab栏列表下对应的文章列表 -->
            <hm-article :post="article" v-for="article in articleList" :key="article.id"></hm-article>
          </van-list>
        </van-pull-refresh>
      </van-tab>
    </van-tabs>
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    console.log("我要离开组件了");
    console.log("to", to);
    // 如果进入的是详情页,home组件依旧要被缓存
    if (to.name !== "article-detail") {
      //如果进入的不是详情页,home组件就不应该被缓存
